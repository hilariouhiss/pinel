...
| 会话树展示 | fork 血缘可视化：宿主解析 header.parentSession（session-history.ts，路径/id 双形态透传）→ webview 纯函数 session-tree.ts（血缘解析/子树排序/防环，session-tree.check.mjs 编译门）→ 两个列表视图（HistoryApp + SessionListPopover）全展开缩进树（↳ 标记、根按子树最新活动排序、点击即 switch_session）；旧树弹层/插件 /pinel-tree 链路不复活；compact 原生 RPC（protocol CompactCommand + controller.compact + 设置面板 Compact now）；阈值 setCompactionThreshold（百分比↔reserveTokens 写全局 settings.json + status.autoCompactPercent 回显） |
