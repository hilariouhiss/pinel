...
| `media/` | 图标（send/settings/stop/history.svg 经 esbuild text loader 内联进 webview bundle、pinel-icon.svg 为扩展图标，均入库）；webview 构建产物（gitignored，构建生成） |
...
- 质量门：`npm run compile` 与 `npm test` 必须全绿（当前 121/121 通过：主套件 118 + 空窗口套件 3）
...
- **SVG 图标链路（send/settings/stop/history.svg）**：文件入库 `media/`（`.gitignore` 仅忽略 webview.js/css/map）；webview 侧经 esbuild `--loader:.svg=text` 以原始文本内联进 bundle（`webview-ui/esbuild.js` 的 loader 配置 + `global.d.ts` 的 `declare module "*.svg"`），`dangerouslySetInnerHTML` 渲染进 DOM（静态自有文件，无注入风险；CSP `img-src data:` 不涉及）；主题自适应靠 **path 级 CSS 选择器**覆盖（`.composer-send .icon path` → `--vscode-button-foreground`、`.composer-stop .icon path` → `--vscode-errorForeground`、`.status-settings-btn .icon path` / `.chat-history-btn .icon path` → `--vscode-descriptionForeground`）——fill 是 `<path>` 上的 presentation attribute，**根元素级选择器无法通过继承覆盖**，必须直接命中 path（specificity 0-2-1）；文件内 `#707070` 保留作兜底；图标 16px 由作用域化规则覆写（文件内硬编码 200px）
