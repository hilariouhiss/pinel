...
- **扩展管理**：footer 卡片 ⚙ 设置按钮旁「扩展」按钮（extension.svg 图标）弹出扩展管理弹层——浏览已安装的 pi 智能体扩展（本地扩展 + packages 分组，含全局/项目徽标与 filtered 标记），每项提供启用/禁用开关与卸载按钮；卸载前确认；任何修改后弹「Reload」确认（点击重启 pi 使变更生效）；不提供安装
