...
- **配置面板**：点击状态栏的模型名/思考等级弹出配置面板——模型/思考强度点击 ↻ 循环切换（切换自下一回合生效，流式中也可切换）；队列模式（steering/跟进：全部投递/一次一条）；自动压缩开关；选择由 pi 持久化，重启 VS Code 后保留；点击外部或 Esc 关闭（面板打开时 Esc 只关面板、不中断对话）
