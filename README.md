...
- **会话分支/回溯**：顶部栏「分支」按钮（fork.svg 图标）弹出历史用户消息选择器——选中某条即从该消息 fork 出新会话并切换过去（原会话保留在历史列表可随时返回），被 fork 消息原文自动回填输入框（可直接发送或编辑后重发，实现“回到这里重新开始”）；弹层底部「Clone current branch」把当前分支复制为新会话（开始新方向前的备份）；数据来自 pi `get_fork_messages`/`fork`/`clone`（0.84.2+）
