...
- **流式控制**：流式输出中按 **Esc**（或点击停止按钮）中断当前操作；流式中继续输入会自动转为 steer 排队；发送/停止按钮使用 lucide 图标（send/square，颜色随主题自适应）
...
- **待办面板**：todo 工具（如 rpiv-todo 插件）的任务列表在输入框上方固定面板展示（可折叠，限高 30vh 内部滚动；折叠后单行显示进行中任务，宽度与消息/输入框对齐）
- **subagent 卡片**：subagent 工具（如 pi-subagents 扩展）的调用渲染为专属卡片——任务描述、所用**模型与思考深度**（继承主会话时显示 main model / main level）、运行中实时活动与轮次/token 统计，完成后点击展开 Markdown 报告全文
...
- **/new 新建会话**：输入框发送 `/new` 直接新建会话（对齐 pi TUI 语义；精确匹配，带参数/附件时原样发送；流式中自动中断并新建；旧会话保留在历史列表可随时切回）
- **聊天界面会话历史**：顶部栏左侧显示当前会话标题，右侧按钮组——「会话历史」按钮（lucide history 图标）弹出会话列表（名称/摘要/相对时间/当前标记，含搜索框），选择即切换会话；「新会话」按钮（lucide plus 图标）一键新建；主侧边栏会话历史面板同样支持搜索，新会话按钮为透明描边样式（lucide plus 图标）
- **会话重命名/删除**：会话列表行右侧操作按钮（hover 显示）——lucide pencil 行内编辑重命名（名称变输入框，Enter 提交 / Esc 取消，当前会话经 pi `set_session_name`、其他会话直接写入会话文件）；lucide trash-2 删除会话（删除前确认弹窗；当前会话不可删除）
- **会话分支/回溯**：顶部栏「分支」按钮（lucide git-fork 图标）弹出历史用户消息选择器——选中某条即从该消息 fork 出新会话并切换过去（原会话保留在历史列表可随时返回），被 fork 消息原文自动回填输入框（可直接发送或编辑后重发，实现“回到这里重新开始”）；弹层底部「Clone current branch」把当前分支复制为新会话（开始新方向前的备份）；数据来自 pi `get_fork_messages`/`fork`/`clone`（0.84.2+）
...
- **扩展管理**：footer 卡片 ⚙ 设置按钮旁「扩展」按钮（lucide puzzle 图标）弹出扩展管理弹层——浏览已安装的 pi 智能体扩展（本地扩展 + packages 分组，含全局/项目徽标与 filtered 标记），每项提供启用/禁用开关与卸载按钮；卸载前确认；任何修改后弹「Reload」确认（点击重启 pi 使变更生效）；不提供安装
