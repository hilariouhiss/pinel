 ...
 * setWidget.widgetLines 内嵌 JSON 字符串）。payload 契约见各采集器头注释
 * （pi/extensions/prompt-composition.ts、mcp-status.ts、pinel-workflows.ts，v:1）。
 * 解析规则对齐 session-stats.ts：结构不符返回 null，
