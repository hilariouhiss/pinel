...
export class ChatController {
  ...
  private handlePinelStatus(req: ExtensionUiRequest): void {
    if (req.statusKey === "pinel.prompt") {
      const parsed = parsePinelPrompt(req.statusText);
      if (!parsed) {
        return;
      }
      this.pinelPromptCache = parsed;
      this.fire({ type: "pinelPrompt", prompt: parsed });
      return;
    }
    ...
    if (req.statusKey === "mcp") {
      // pi-mcp-adapter footer 状态帧（ANSI 装饰 "🔌 MCP: 3 servers enabled (2 connected)" /
      // compact "MCP 2/3" / undefined 清除）；解析失败（形状漂移）静默丢帧
      const parsed = parseMcpStatus(req.statusText);
      if (!parsed) {
        return;
      }
      this.mcpStatusCache = parsed;
  ...
  fireSnapshot(): void {
    this.fire({
      ...
      pinelPrompt: this.pinelPromptCache,
  ...
  /** 最近一次 pinel.prompt 推送缓存（集成测试断言）。 */
  getPinelPromptCache(): PinelPromptPayload | null {
    return this.pinelPromptCache;
  }
